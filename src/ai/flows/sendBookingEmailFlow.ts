...
'use server';
/**
 * @fileOverview A Genkit flow to send booking confirmation emails.
 *
...
export async function sendBookingConfirmationEmail(input: BookingConfirmationEmailInput): Promise<{ success: boolean; message: string }> {
  console.log("sendBookingConfirmationEmail: Flow invoked with input (password omitted):", {
    ...input,
...
const bookingEmailFlow = ai.defineFlow(
  ...
  async (bookingDetails) => {
    try { // Added top-level try-catch within the flow
      ...
      console.log("  Pass Present:", !!smtpPass);
      ...
      const customerEmailSubject = `Your FixBro Booking Confirmed! (ID: ${bookingDetails.bookingId})`;
      ...
      const adminEmailSubject = `New Booking Received (ID: ${bookingDetails.bookingId})`;
      ...
      if (!canAttemptRealEmail) {
        console.warn("SMTP configuration incomplete or missing. Required fields: smtpHost, smtpPort, smtpUser, smtpPass, senderEmail. Falling back to console logging for emails.");
        console.log("\n--- SIMULATING Email to Customer ---");
        console.log(`To: ${bookingDetails.customerEmail}`);
        console.log(`Subject: ${customerEmailSubject}`);
        console.log(`Body:\n${customerEmailBody}`);
        console.log("\n--- SIMULATING Email to Admin ---");
      ...
      try {
        console.log(`Attempting to send email to customer: ${bookingDetails.customerEmail} from FixBro <${senderEmail}>`);
        ...
        console.log(`Attempting to send email to admin: ${adminEmail} from FixBro Admin <${senderEmail}>`);
        ...
        console.log("\n--- FAILED TO SEND REAL EMAIL - Simulating Email to Customer (Fallback) ---");
        console.log(`To: ${bookingDetails.customerEmail}`);
        console.log(`Subject: ${customerEmailSubject}`);
        console.log(`Body:\n${customerEmailBody}`);
        console.log("\n--- FAILED TO SEND REAL EMAIL - Simulating Email to Admin (Fallback) ---");
        console.log(`To: ${adminEmail}`);
        console.log(`Subject: ${adminEmailSubject}`);
        console.log(`Body:\n${adminEmailBody}`);
    ...
    } catch (flowError) { // Catch any unexpected error within the flow execution
      ...
      // Ensure a structured response is always returned from the flow itself.
