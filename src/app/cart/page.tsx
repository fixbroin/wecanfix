...
import { getGuestId } from '@/lib/guestIdManager'; // Import guest ID manager
import { useAuth as useAuthHook } from '@/hooks/useAuth'; // Renamed to avoid conflict with 'auth' from firebase
...
export default function CartPage() {
  ...
  const { user } = useAuthHook(); // Use the renamed hook
  ...
  const [estimatedTax, setEstimatedTax] = useState(0); // Total tax on items + VC
  const [total, setTotal] = useState(0); // Grand total
        ...
        const loadedCartItemsPromises = entries.map(async (entry) => {
          ...
          if (serviceSnap.exists()) {
            ...
            return {
              ...
              price,
              isTaxInclusive: serviceData.isTaxInclusive === true, // Explicitly check for boolean true
              discountedPrice,
  ...
  useEffect(() => {
    ...
    setSubtotal(currentBaseSubtotalFromItems); // This is sum of BASE item prices
    ...
    // Grand Total: (Sum of BASE item prices) + (BASE visiting charge) + (Total calculated tax)
  ...
  const handleQuantityChange = (itemId: string, newQuantity: number) => {
    ...
    if (newQuantity === 0) {
      ...
      logUserActivity(
        'removeFromCart',
        { serviceId: itemBeingChanged.id, serviceName: itemBeingChanged.name, quantity: itemBeingChanged.quantity /* old quantity */ },
        user?.uid,
            ...
            {cartItems.map(item => (
              <Card key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center p-3 sm:p-4 shadow-sm">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-sm sm:text-md font-medium text-primary mt-1">
                    ₹{((typeof item.discountedPrice === 'number' ? item.discountedPrice : item.price) || 0).toLocaleString()}
                    {item.isTaxInclusive && item.taxPercent && item.taxPercent > 0 && <span className="text-xs text-muted-foreground ml-1">(incl. tax)</span>}
                    {!item.isTaxInclusive && item.taxPercent && item.taxPercent > 0 && <span className="text-xs text-muted-foreground ml-1">(+{item.taxPercent}% tax)</span>}
                  </p>
                </div>
                <div className="flex flex-col items-stretch sm:items-end mt-3 sm:mt-0 space-y-2 sm:space-y-0 sm:ml-4 w-full sm:w-auto">
                  <QuantitySelector
                    ...
                    minQuantity={0} // Allow setting to 0 to remove
                  ...
                  <Button
                    ...
                    onClick={() => handleRemoveItem(item.id)} // This specific remove button can be simpler if minQuantity is 0
                  >
                    ...
                    <Dialog open={isTaxBreakdownOpen} onOpenChange={setIsTaxBreakdownOpen}>
                        ...
                        <DialogContent className="w-[90vw] sm:max-w-md max-h-[80vh] overflow-y-auto">
                            ...
                            <TaxBreakdownDisplay
                                ...
                                subTotalBeforeDiscount={subtotal} // Sum of BASE item prices
                                totalDiscount={0} // Assuming no cart-level discount for now
