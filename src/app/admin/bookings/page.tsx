...
import { Tag, Eye, Loader2, PackageSearch, XIcon, Edit, Trash2, CalendarDays, Clock } from "lucide-react"; 
import type { FirestoreBooking, BookingStatus, BookingServiceItem, AppSettings } from '@/types/firestore';
...
const formatDateForDisplay = (dateString: string | undefined): string => {
    ...
    try {
        ...
        const date = new Date(dateString.replace(/-/g, '/')); // Ensure correct parsing
...
export default function AdminBookingsPage() {
  ...
  const prepareAndSendEmail = async (
    ...
    if (emailType === 'booking_completion') {
      try {
        const dataUri = await generateInvoicePdf(booking, companyDetailsForInvoice);
        if (typeof dataUri === 'string' && dataUri.includes(',')) {
          invoicePdfBase64 = dataUri.split(',')[1];
        } else {
          console.warn("generateInvoicePdf did not return a valid data URI string for completion email.");
        }
      } catch (invoiceError) {
        console.error("Error generating invoice for completion email:", invoiceError);
      }
    }
    ...
    const emailInput: BookingConfirmationEmailInput = {
      ...
      scheduledDate: formatDateForDisplay(booking.scheduledDate),
      scheduledTimeSlot: booking.scheduledTimeSlot,
      services: booking.services.map((s: BookingServiceItem) => ({
        serviceId: s.serviceId, name: s.name, quantity: s.quantity,
        pricePerUnit: s.pricePerUnit, discountedPricePerUnit: s.discountedPricePerUnit,
      })),
      subTotal: booking.subTotal, 
      visitingCharge: booking.visitingCharge, 
      discountAmount: booking.discountAmount,
      discountCode: booking.discountCode,
      appliedPlatformFees: booking.appliedPlatformFees?.map(fee => ({
        name: fee.name, amount: fee.calculatedFeeAmount + fee.taxAmountOnFee,
      })),
      taxAmount: booking.taxAmount,
      totalAmount: booking.totalAmount,
      paymentMethod: updatedPaymentMethod || booking.paymentMethod,
      ...
      previousScheduledDate: originalScheduledDateForEmail ? formatDateForDisplay(originalScheduledDateForEmail) : undefined,
      previousScheduledTimeSlot: originalScheduledTimeSlotForEmail
    };

    try {
      const emailResult = await sendBookingConfirmationEmail(emailInput);
      if (emailResult.success) {
        toast({ title: "Email Sent", description: `${emailType.replace('_', ' ')} email sent to customer.` });
      } else {
        toast({ title: "Email Failed", description: emailResult.message || `Could not send ${emailType.replace('_', ' ')} email.`, variant: "destructive", duration: 7000 });
      }
      return emailResult.success;
    } catch (emailFlowError) {
      console.error(`Error calling sendBookingConfirmationEmail flow for ${emailType}:`, emailFlowError);
      toast({ title: "Email System Error", description: "Failed to trigger email sending process.", variant: "destructive", duration: 7000 });
      return false;
    }
  };
  ...
  const handleStatusChange = async (booking: FirestoreBooking, newStatus: BookingStatus) => {
    ...
    if (newStatus === "Rescheduled" && booking.status !== "Rescheduled") {
      ...
      const currentScheduledDate = new Date(booking.scheduledDate.replace(/-/g, '/')); // Ensure correct parsing for various date formats
      setRescheduleDate(isNaN(currentScheduledDate.getTime()) ? new Date() : currentScheduledDate);
      setRescheduleSelectedTimeSlot(booking.scheduledTimeSlot);
      setIsRescheduleDialogOpen(true);
      fetchRescheduleSlots(isNaN(currentScheduledDate.getTime()) ? new Date() : currentScheduledDate); 
      return;
    }
    ...
    if (requiresPaymentMethodConfirmation && newStatus === "Completed") {
      ...
      try {
        ...
        if (newStatus === "Completed" && booking.isReviewedByCustomer === undefined) {
            updatePayload.isReviewedByCustomer = false;
        }
        await updateDoc(bookingDocRef, updatePayload);
        toast({ title: "Success", description: `Booking status updated to ${newStatus}.` });

        if (newStatus === "Completed") {
            const bookingForEmail = { ...booking, status: newStatus };
            await prepareAndSendEmail(bookingForEmail, 'booking_completion');
        }
      } catch (error) {
  ...
  const handleConfirmPaymentAndUpdateStatus = async () => {
    ...
    const bookingToUpdate = selectedBookingForPaymentUpdate;
    const bookingIdToUpdate = bookingToUpdate.id;

    setIsUpdatingStatus(bookingIdToUpdate);
    setIsPaymentMethodDialogOpen(false);
    
    const updatedBookingForEmail: FirestoreBooking = {
        ...bookingToUpdate,
        paymentMethod: paymentReceivedMethodForDialog,
        status: "Completed" 
    };
    ...
    if (bookingToUpdate.isReviewedByCustomer === undefined) {
        updatePayload.isReviewedByCustomer = false;
    }
  ...
  const generateAdminRawTimeSlots = (referenceDate: Date, config: AppSettings): string[] => {
    ...
    if (config.enableLimitLateBookings && typeof config.limitLateBookingHours === 'number' && config.limitLateBookingHours > 0) {
        effectiveDelayHours = config.limitLateBookingHours;
    }
    ...
    servicePeriodsToUse.forEach(periodConfig => {
        ...
        while (currentSlotTimeMinutes < periodEndTimeMinutes) {
            ...
            currentSlotTimeMinutes += slotInterval;
        }
    });
    return slots;
  };
  ...
  const fetchRescheduleSlots = async (date: Date) => {
    if (!appConfig || isLoadingAppSettings) {
        toast({ title: "Config Loading", description: "Application settings are still loading. Please wait.", variant: "default" });
        return;
    }
    ...
    if (selectedDay < today) {
      ...
      toast({ title: "Invalid Date", description: "Cannot reschedule to a past date.", variant: "destructive" });
      return;
    }
    
    const rawSlots = generateAdminRawTimeSlots(date, appConfig);
    setRescheduleAvailableTimeSlots(rawSlots);
    setIsLoadingRescheduleSlots(false);
  };

  useEffect(() => {
    if (isRescheduleDialogOpen && rescheduleDate && selectedBookingForReschedule) {
      fetchRescheduleSlots(rescheduleDate);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rescheduleDate, isRescheduleDialogOpen, selectedBookingForReschedule, appConfig]);

  const handleRescheduleDateSelect = (date: Date | undefined) => {
    if (date) {
      setRescheduleDate(date);
      setRescheduleSelectedTimeSlot(undefined); 
    }
  };

  const handleConfirmReschedule = async () => {
    if (!selectedBookingForReschedule || !selectedBookingForReschedule.id || !rescheduleDate || !rescheduleSelectedTimeSlot) {
      toast({ title: "Missing Information", description: "Please select a new date and time slot.", variant: "destructive" });
      return;
    }
    setIsUpdatingStatus(selectedBookingForReschedule.id);
    const bookingDocRef = doc(db, "bookings", selectedBookingForReschedule.id);
    const originalScheduledDate = selectedBookingForReschedule.scheduledDate; // Store for email
    const originalScheduledTimeSlot = selectedBookingForReschedule.scheduledTimeSlot; // Store for email
    try {
      const newScheduledDate = rescheduleDate.toLocaleDateString('en-CA'); // YYYY-MM-DD for Firestore
      await updateDoc(bookingDocRef, {
        scheduledDate: newScheduledDate,
        scheduledTimeSlot: rescheduleSelectedTimeSlot,
      ...
      toast({ title: "Booking Rescheduled", description: `Booking ${selectedBookingForReschedule.bookingId} has been rescheduled.` });
      setIsRescheduleDialogOpen(false);
      setSelectedBookingForReschedule(null);
    } catch (error) {
      console.error("Error rescheduling booking:", error);
      toast({ title: "Error", description: "Could not reschedule booking.", variant: "destructive" });
    } finally {
      setIsUpdatingStatus(null);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <br />
              Select the method used to receive payment. This will mark the booking as "Completed".
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-2">
            ...
            <Button
              ...
              disabled={!paymentReceivedMethodForDialog || isUpdatingStatus === selectedBookingForPaymentUpdate?.id || isLoadingAppSettings}
            >
              {(isUpdatingStatus === selectedBookingForPaymentUpdate?.id || isLoadingAppSettings) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
      ...
      <Dialog open={isRescheduleDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-lg">
            <DialogHeader>
                <DialogTitle>Reschedule Booking: {selectedBookingForReschedule?.bookingId}</DialogTitle>
                <DialogDescription>
                    Select a new date and time for this booking. Current: {selectedBookingForReschedule ? formatDateForDisplay(selectedBookingForReschedule.scheduledDate) : ''} at {selectedBookingForReschedule?.scheduledTimeSlot}.
                </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
                <div>
                    <Label htmlFor="reschedule-calendar" className="block mb-2 font-medium">New Date</Label>
                    <Calendar
                        mode="single"
                ...
                {rescheduleDate && (
                <div>
                    <Label className="block mb-2 font-medium">Available Time Slots for {rescheduleDate.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' })}</Label>
                    {isLoadingRescheduleSlots ? (
                         <div className="flex items-center justify-center py-2"><Loader2 className="h-5 w-5 animate-spin mr-2"/> Fetching slots...</div>
                    ) : rescheduleAvailableTimeSlots.length > 0 ? (
                        <RadioGroup value={rescheduleSelectedTimeSlot} onValueChange={setRescheduleSelectedTimeSlot} className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {rescheduleAvailableTimeSlots.map(slot => (
                                <Label key={slot} htmlFor={`slot-${slot}`} className={`flex items-center justify-center space-x-2 border rounded-md p-2 hover:bg-accent/50 cursor-pointer ${rescheduleSelectedTimeSlot === slot ? 'bg-primary text-primary-foreground border-primary ring-1 ring-primary' : 'border-input bg-background'}`}>
                                    <RadioGroupItem value={slot} id={`slot-${slot}`} className="border-muted-foreground data-[state=checked]:border-primary-foreground" />
            ...
            <DialogFooter>
                ...
                <Button onClick={handleConfirmReschedule} disabled={!rescheduleDate || !rescheduleSelectedTimeSlot || isLoadingRescheduleSlots || isUpdatingStatus === selectedBookingForReschedule?.id}>
                     {(isUpdatingStatus === selectedBookingForReschedule?.id || isLoadingRescheduleSlots) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    Confirm Reschedule
                </Button>
            </DialogFooter>
        </DialogContent>
