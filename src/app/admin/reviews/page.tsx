...
import { PlusCircle, Edit, Trash2, Loader2, Star, MessageSquare, Filter } from "lucide-react"; 
import type { FirestoreReview, ReviewStatus, FirestoreService } from '@/types/firestore';
...
export default function AdminReviewsPage() {
  ...
  const [services, setServices] = useState<Pick<FirestoreService, 'id' | 'name'>[]>([]);
  ...
  useEffect(() => {
    let servicesFetchedSuccessfully = false;
    let initialReviewsSnapshotReceived = false;
    let initialServicesFetchAttempted = false;
    let initialReviewsFetchAttempted = false;
    let serviceUnsubscribe = () => {};
    let reviewUnsubscribe = () => {};

    const tryStopLoading = () => {
      if ((servicesFetchedSuccessfully || initialServicesFetchAttempted) && 
          (initialReviewsSnapshotReceived || initialReviewsFetchAttempted)) {
        setIsLoading(false);
      }
    };

    const fetchServicesAsync = async () => {
      initialServicesFetchAttempted = true;
      try {
        const servicesQuery = query(collection(db, "adminServices"), orderBy("name"));
        serviceUnsubscribe = onSnapshot(servicesQuery, (servicesSnapshot) => {
            setServices(servicesSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name as string })));
            servicesFetchedSuccessfully = true;
            tryStopLoading();
        }, (error) => {
            console.error("Error fetching services for review form: ", error);
            toast({ title: "Error", description: "Could not load services list for the form.", variant: "destructive" });
            tryStopLoading();
        });
      } catch (error) {
        console.error("Error setting up services listener: ", error);
        toast({ title: "Error", description: "Could not set up services listener.", variant: "destructive" });
        tryStopLoading();
      }
    };
    ...
    reviewUnsubscribe = onSnapshot(qReviews, (querySnapshot) => {
      initialReviewsFetchAttempted = true; 
      const fetchedReviews = querySnapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id } as FirestoreReview));
      setReviews(fetchedReviews);
      if (!initialReviewsSnapshotReceived) {
        initialReviewsSnapshotReceived = true;
      }
      tryStopLoading(); 
    }, (error) => {
      initialReviewsFetchAttempted = true; 
    ...
    return () => {
        if (typeof serviceUnsubscribe === 'function') serviceUnsubscribe();
        if (typeof reviewUnsubscribe === 'function') reviewUnsubscribe();
    };
  }, [toast]);
