...
export default function AdminPromoCodesPage() {
  ...
  const handleToggleActive = async (code: FirestorePromoCode) => {
    ...
    try {
      ...
      fetchPromoCodes(); // Re-fetch to update UI state, or update locally
    ...
    <div className="space-y-6">
                  ...
                  promoCodes.map((code) => (
                    <TableRow key={code.id}>
                      ...
                      <TableCell className="text-center text-xs">
                        {code.validFrom ? formatDate(code.validFrom) : <XCircle className="h-4 w-4 text-muted-foreground/70 mx-auto" title="Not set"/>}
                      </TableCell>
                      <TableCell className="text-center text-xs">
                        {code.validUntil ? formatDate(code.validUntil) : <XCircle className="h-4 w-4 text-muted-foreground/70 mx-auto" title="Not set"/>}
                      </TableCell>
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => { if (!isSubmitting) { setIsFormOpen(open); if (!open) setEditingPromoCode(null); } }}>
        <DialogContent className="w-[90vw] max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl max-h-[90vh] p-0 flex flex-col overflow-y-auto">
          ...
          {/* PromoCodeForm is responsible for its own internal scrolling if needed via its structure */}
          <PromoCodeForm
