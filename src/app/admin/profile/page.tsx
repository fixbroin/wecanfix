...
import { Label } from "@/components/ui/label"; // Direct import, not from form
...
const ADMIN_EMAIL = "fixbro.in@gmail.com"; // Ensure this is consistent
  ...
  const handleImageFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 2 * 1024 * 1024) { // 2MB limit
        ...
        setProfileImageFile(null); // Clear the state
        setImagePreview(currentPhotoURL); // Revert preview to original/current
        return;
      }
  ...
  const handleProfileUpdate = async (data: ProfileFormData) => {
    ...
    if (profileImageFile) {
      ...
      try {
        ...
        setIsSavingProfile(false); // Ensure main button is re-enabled
        setProfileImageFile(null); // Clear selected file from state
        if(fileInputRef.current) fileInputRef.current.value = ""; // Clear file input visually
        setImagePreview(currentPhotoURL); // Revert preview to what it was before attempting upload
        return; 
      } finally {
        // This finally runs after the try/catch block for image upload
        setIsUploading(false);
        setUploadProgress(null);
      }
    }
    ...
    // 2. Update Firebase Auth Profile (Name and new PhotoURL)
    try {
      await updateAuthProfile(auth.currentUser, {
        displayName: data.displayName,
        // Only include photoURL if it actually changed (either new upload or manually cleared)
      ...
      // Note: Image might have been uploaded. This state is tricky.
      // For simplicity, we don't revert image upload here, but a more robust solution might.
      setIsSavingProfile(false); // Still need to reset this
      return;
    }
    ...
    try {
      ...
      setIsSavingProfile(false); // Ensure this is always called at the end
    }
  };
