...
export default function AdminServicesPage() {
  ...
  const handleFormSubmit = async (data: Omit<FirestoreService, 'id' | 'createdAt'> & { id?: string }) => {
    ...
    const payloadForFirestore: Partial<FirestoreService> = {
      name: data.name, slug: finalSlugForSave, subCategoryId: data.subCategoryId, description: data.description,
      price: Number(data.price), isTaxInclusive: data.isTaxInclusive, 
      discountedPrice: (data.discountedPrice !== undefined && data.discountedPrice !== null) ? Number(data.discountedPrice) : undefined,
      rating: Number(data.rating || 0), reviewCount: Number(data.reviewCount || 0), isActive: data.isActive === undefined ? true : data.isActive,
      imageUrl: data.imageUrl || "", imageHint: data.imageHint || "", shortDescription: data.shortDescription || undefined,
      fullDescription: data.fullDescription || undefined, serviceHighlights: data.serviceHighlights || [],
      taxId: data.taxId, taxName: selectedTax?.taxName, taxPercent: selectedTax?.taxPercent,
    ...
    try {
      if (editingService && data.id) {
        ...
        delete (updateData as any).id; await updateDoc(serviceDoc, updateData);
