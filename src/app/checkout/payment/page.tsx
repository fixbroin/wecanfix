...
export default function PaymentPage() {
      ...
      try {
        const detailsPromises = currentCartEntries.map(async (entry) => {
          ...
          if (serviceSnap.exists()) {
            ...
            return { id: serviceSnap.id, ...serviceData, isTaxInclusive: serviceData.isTaxInclusive === true } as FirestoreService;
          }
  ...
  useEffect(() => {
    if (!isLoadingCartDetails && !isLoadingAppSettings) {
      ...
      if (onlinePaymentEnabled) setPaymentMethod("upi"); else if (payAfterServiceEnabled) setPaymentMethod("later"); else setPaymentMethod("");
    }
  ...
  const basePaymentOptions = [
    ...
    { value: 'later', label: 'Pay After Service', icon: HandCoins, online: false },
  ];
  const availablePaymentOptions = basePaymentOptions.filter(option => {
      if (isCancellationFeeMode) return option.online && onlinePaymentEnabled; 
      return option.online ? onlinePaymentEnabled : payAfterServiceEnabled;
  });
      ...
      <Card className="shadow-lg">
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between gap-2 mt-4">
          ...
          <Button
            ...
            disabled={ (isCancellationFeeMode ? !cancellationFeeDetails : cartEntries.length === 0) || isLoadingCartDetails || isLoadingAppSettings || availablePaymentOptions.length === 0 || (!paymentMethod && !isCancellationFeeMode) || isProcessingPayment}
