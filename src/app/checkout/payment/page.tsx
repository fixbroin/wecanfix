...
import { ArrowRight, ArrowLeft, CreditCard, Landmark, IndianRupee, Wallet, Info, Clock, Loader2, Tag, CheckCircle, XCircle, ListFilter, HandCoins } from 'lucide-react';
...
export default function PaymentPage() {
  ...
  const { user: currentUser } = useAuth(); // For logging
  ...
  const [subTotal, setSubTotal] = useState(0); // Sum of BASE prices of items
  const [visitingCharge, setVisitingCharge] = useState(0); // BASE visiting charge
  const [taxAmount, setTaxAmount] = useState(0); // Total tax on items + VC + Platform Fees
  const [totalAmountDue, setTotalAmountDue] = useState(0); // Grand total
  ...
  const payAfterServiceEnabled = useMemo(() => appConfig.enableCOD !== false, [appConfig]);
  ...
  useEffect(() => {
    ...
    console.log("[PaymentPage] Component mounted or AppSettings loaded. Starting initial data load.");
    ...
    const loadInitialData = async () => {
      ...
      logUserActivity(
        'checkoutStep',
        { checkoutStepName: 'payment', pageUrl: pathname, cartItemCount: currentCartEntries.length },
        currentUser?.uid,
      ...
      try {
        ...
        const detailsMap = resolvedDetails.reduce((acc, service) => {
        ...
        if (validCartEntries.length !== currentCartEntries.length) {
            setCartEntries(validCartEntries);
        }

        const storedPromo = localStorage.getItem('fixbroAppliedPromoCode');
        if (storedPromo) {
            try {
                const parsedPromo: AppliedPromoCodeInfo = JSON.parse(storedPromo);
                setAppliedPromoCode(parsedPromo);
                setPromoCodeInput(parsedPromo.code);
            } catch (e) {
        ...
        console.error("[PaymentPage] Error fetching service details or promos:", error);
        toast({ title: "Error", description: "Could not load page data. Please try refreshing.", variant: "destructive" });
        setAllFetchedPromoCodes([]);
      } finally {
        setIsLoadingCartDetails(false);
      }
    };
    loadInitialData();
  }, [isMounted, toast, isLoadingAppSettings, appConfig, pathname, currentUser]);

  useEffect(() => {
    if (isLoadingCartDetails || isLoadingAppSettings || cartEntries.length === 0) {
      if (cartEntries.length === 0 && !isLoadingCartDetails && !isLoadingAppSettings) {
    ...
    let allItemsHaveSameTaxRate = true;
    let firstItemTaxRate: number | undefined = undefined;
    ...
    cartEntries.forEach((entry, index) => {
      ...
      if (detail) {
        ...
        if (index === 0) firstItemTaxRate = itemTaxRatePercent;
        else if (itemTaxRatePercent !== firstItemTaxRate) allItemsHaveSameTaxRate = false;

        newBreakdownItems.push({
            name: detail.name,
            quantity: entry.quantity,
            pricePerUnit: displayedPricePerUnit,
            itemSubtotal: basePricePerUnitForItem * entry.quantity,
    ...
    if (appliedPromoCode && currentSumOfDisplayedPrices > 0) {
        if (appliedPromoCode.discountType === 'percentage') {
            currentDiscountAmount = (currentSumOfDisplayedPrices * appliedPromoCode.discountValue) / 100;
        } else {
            currentDiscountAmount = appliedPromoCode.discountValue;
        }
    ...
    if (appConfig.enableMinimumBookingPolicy && typeof appConfig.minimumBookingAmount === 'number' && typeof appConfig.visitingChargeAmount === 'number') {
      ...
      if (appConfig.minimumBookingPolicyDescription) {
        currentPolicyMessage = appConfig.minimumBookingPolicyDescription
          .replace(/{MINIMUM_BOOKING_AMOUNT}/g, appConfig.minimumBookingAmount.toString())
          .replace(/{VISITING_CHARGE}/g, (appConfig.visitingChargeAmount || 0).toString());
      }
    }
    ...
    let runningTotalForPlatformFeeBase = 0;
    let runningTotalTaxOnPlatformFees = 0;
    const newCalculatedPlatformFees: AppliedPlatformFeeItem[] = [];
        ...
        appConfig.platformFees.forEach(fee => {
            if (fee.isActive) {
                ...
                if (fee.type === 'percentage') {
                    // Calculate percentage fee on sum of DISPLAYED item prices (before cart discount)
                    feeBaseAmount = (currentSumOfDisplayedPrices * fee.value) / 100;
                } else { // fixed
                    feeBaseAmount = fee.value;
                }
                const taxOnThisFee = feeBaseAmount * (fee.feeTaxRatePercent / 100);

                newCalculatedPlatformFees.push({
                    name: fee.name,
                    type: fee.type,
                    valueApplied: fee.value,
                    calculatedFeeAmount: feeBaseAmount,
                    taxRatePercentOnFee: fee.feeTaxRatePercent,
                    taxAmountOnFee: taxOnThisFee,
                });
                runningTotalForPlatformFeeBase += feeBaseAmount;
                runningTotalTaxOnPlatformFees += taxOnThisFee;
            }
        });
    }
    setCalculatedPlatformFees(newCalculatedPlatformFees);
    setTotalPlatformFeeBaseAmount(runningTotalForPlatformFeeBase);
    setTotalTaxOnPlatformFees(runningTotalTaxOnPlatformFees);
    ...
    const finalTotalTaxAmount = totalItemTaxAmount + visitingChargeTaxAmount + runningTotalTaxOnPlatformFees;
    setTaxAmount(finalTotalTaxAmount);

    let effectiveGlobalRate = 0;
    if (allItemsHaveSameTaxRate && firstItemTaxRate !== undefined) effectiveGlobalRate = firstItemTaxRate;
    if (calculatedBaseVisitingCharge > 0 && appConfig.enableTaxOnVisitingCharge && (appConfig.visitingChargeTaxPercent || 0) > 0) {
        if (!(allItemsHaveSameTaxRate && (appConfig.visitingChargeTaxPercent || 0) === firstItemTaxRate)) allItemsHaveSameTaxRate = false;
    } else if (calculatedBaseVisitingCharge > 0 && (!appConfig.enableTaxOnVisitingCharge || (appConfig.visitingChargeTaxPercent || 0) <= 0)) {
        if (allItemsHaveSameTaxRate && firstItemTaxRate !== 0) allItemsHaveSameTaxRate = false;
    }
    // Consider platform fees tax for effective rate display if needed, for now focusing on item/VC tax
    if (newCalculatedPlatformFees.some(pf => pf.taxRatePercentOnFee > 0)) {
        allItemsHaveSameTaxRate = false; // If any platform fee has tax, overall rate is mixed
    }


    if (allItemsHaveSameTaxRate && effectiveGlobalRate > 0) setEffectiveTaxRateDisplay(`Tax (${effectiveGlobalRate.toFixed(1)}%)`);
    else if (finalTotalTaxAmount > 0) setEffectiveTaxRateDisplay("Total Tax");
    else setEffectiveTaxRateDisplay("Tax (0%)");
    ...
    if (allFetchedPromoCodes.length === 0) {
      setAvailablePromoCodesToDisplay([]);
    ...
    const filtered = allFetchedPromoCodes.filter(promoData => {
      ...
      if (validFrom) { const start = new Date(validFrom); start.setHours(0,0,0,0); if (currentDate < start) isValid = false; }
      const validUntil = promoData.validUntil?.toDate();
      if (isValid && validUntil) { const end = new Date(validUntil); end.setHours(23,59,59,999); if (currentDate > end) isValid = false; }
    ...
    setAvailablePromoCodesToDisplay(filtered);
    setIsLoadingPromos(false);

  }, [cartEntries, serviceDetailsMap, appConfig, isLoadingCartDetails, isLoadingAppSettings, appliedPromoCode, allFetchedPromoCodes]);
  ...
  }, [isLoadingCartDetails, isLoadingAppSettings, onlinePaymentEnabled, payAfterServiceEnabled]);

  const handleApplyPromoCode = async () => {
    if (!promoCodeInput.trim()) {
      toast({ title: "Info", description: "Please enter a promo code.", variant: "default" });
      return;
    }
    setIsApplyingPromo(true);
    setAppliedPromoCode(null);
    setDiscountAmount(0); // Reset discount before applying new one

    try {
      const promoCodeRef = collection(db, "adminPromoCodes");
      const q = query(promoCodeRef, where("code", "==", promoCodeInput.toUpperCase()));
      ...
      const promoData = { id: promoDoc.id, ...promoDoc.data() } as FirestorePromoCode;
      const currentDate = new Date();

      if (!promoData.isActive) {
        toast({ title: "Inactive Code", description: "This promo code is currently not active.", variant: "destructive" });
        setIsApplyingPromo(false); return;
      }

      const validFrom = promoData.validFrom?.toDate();
      if (validFrom) {
          const start = new Date(validFrom); start.setHours(0,0,0,0);
          if (currentDate < start) { toast({ title: "Not Yet Valid", description: "This promo code is not active yet.", variant: "destructive" }); setIsApplyingPromo(false); return; }
      }
      const validUntil = promoData.validUntil?.toDate();
      if (validUntil) {
        const end = new Date(validUntil); end.setHours(23,59,59,999);
        if (currentDate > end) { toast({ title: "Expired Code", description: "This promo code has expired.", variant: "destructive" }); setIsApplyingPromo(false); return; }
      }
      if (promoData.minBookingAmount && sumOfDisplayedItemPrices < promoData.minBookingAmount) {
        toast({ title: "Minimum Amount Not Met", description: `Minimum booking amount of ₹${promoData.minBookingAmount} required. Your items total is ₹${sumOfDisplayedItemPrices.toFixed(2)}.`, variant: "destructive" });
        setIsApplyingPromo(false); return;
      }
      ...
      if (promoData.discountType === 'percentage') {
        // Calculate discount on the sum of DISPLAYED item prices
        calculatedDiscount = (sumOfDisplayedItemPrices * promoData.discountValue) / 100;
      } else {
        calculatedDiscount = promoData.discountValue;
      }
      calculatedDiscount = Math.min(calculatedDiscount, sumOfDisplayedItemPrices);

      const appliedInfo: AppliedPromoCodeInfo = {
        id: promoData.id,
        code: promoData.code,
      ...
      console.error("[PaymentPage] Error applying promo code:", error);
  ...
  const handleSelectAvailablePromo = (code: string) => {
  ...
  const handlePaymentAction = async () => {
    ...
    if (!onlinePaymentEnabled || !appConfig.razorpayKeyId) {
        toast({ title: "Online Payments Disabled", description: "Online payments are currently not available or configured.", variant: "destructive" });
    ...
    try {
      ...
      if (customerAddressDataString) {
          try { const addr = JSON.parse(customerAddressDataString); customerName = addr.fullName || customerName; customerEmail = addr.email || customerEmail; customerContact = addr.phone || undefined; }
          catch (e) { console.error("Error parsing address for Razorpay:", e); }
      ...
      const options = {
        ...
        name: appConfig.websiteName || "FixBro Services", description: "Service Booking Payment",
        ...
        notes: { address: "FixBro Service Booking", cart_item_count: cartEntries.length.toString(), applied_promo_code: appliedPromoCode?.code || "N/A" },
        theme: { color: "#45A0A2" },
      ...
      rzp.on('payment.failed', (response: any) => {
        ...
        setIsProcessingPayment(false); hideLoading();
      });
      rzp.open();
    } catch (error) {
  ...
  if (!isMounted || isLoadingCartDetails || isLoadingAppSettings) {
     return (
      <div className="max-w-2xl mx-auto">
        ...
        <CheckoutStepper currentStepId="payment" />
        <Card className="shadow-lg">
          ...
          <CardFooter className="flex flex-col sm:flex-row justify-between gap-2 mt-4"><Button variant="outline" disabled className="w-full sm:w-auto"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Address</Button><Button size="lg" disabled className="w-full sm:w-auto"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Processing...</Button></CardFooter>
        </Card>
      </div>
    );
  }
  ...
  const basePaymentOptions = [
    { value: 'upi', label: 'UPI', icon: IndianRupee, online: true },
    { value: 'card', label: 'Credit/Debit Card', icon: CreditCard, online: true },
    { value: 'netbanking', label: 'Net Banking', icon: Landmark, online: true },
    { value: 'wallet', label: 'Wallets', icon: Wallet, online: true },
    { value: 'Pay After Service', label: 'Pay After Service', icon: HandCoins, online: false },
  ];
  const availablePaymentOptions = basePaymentOptions.filter(option => option.online ? onlinePaymentEnabled : payAfterServiceEnabled);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <CheckoutStepper currentStepId="payment" />
      <Card className="shadow-lg">
        <CardHeader><CardTitle className="text-2xl font-headline text-center">Complete Your Booking</CardTitle><CardDescription className="text-center">Choose payment method. All online transactions are secure.</CardDescription></CardHeader>
        <CardContent className="space-y-6">
          {onlinePaymentEnabled && paymentMethod !== 'Pay After Service' && cartEntries.length > 0 && (
          ...
          {cartEntries.length > 0 && (
            <>
              ...
              <div className="border-t pt-4 space-y-2 mt-4">
                 <div className="flex justify-between"><span className="text-muted-foreground">Items Total (Displayed):</span><span>₹{sumOfDisplayedItemPrices.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                {discountAmount > 0 && (<div className="flex justify-between text-green-600"><span>Discount ({appliedPromoCode?.code || 'Applied'}):</span><span>- ₹{discountAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>)}
                 ...
                 {calculatedPlatformFees.map((fee, index) => (
                    <div key={index} className="flex justify-between">
                        <span className="text-muted-foreground flex items-center">
                            <HandCoins className="mr-1 h-3.5 w-3.5 text-muted-foreground"/> {fee.name}
                            {fee.taxRatePercentOnFee > 0 && <span className="text-xs ml-1">(incl. tax)</span>}
                        </span>
                 ...
                 <div className="flex justify-between items-center">
                    <div className="flex items-center text-muted-foreground">{effectiveTaxRateDisplay}
                      <Dialog open={isTaxBreakdownOpen} onOpenChange={setIsTaxBreakdownOpen}>
                          <DialogTrigger asChild><Button variant="ghost" size="icon" className="h-5 w-5 ml-1 p-0"><Info className="h-3.5 w-3.5 text-muted-foreground hover:text-primary"/></Button></DialogTrigger>
                          <DialogContent className="w-[90vw] sm:max-w-md max-h-[80vh] overflow-y-auto">
                              ...
                              <TaxBreakdownDisplay items={taxBreakdownItems} visitingCharge={visitingChargeBreakdown} platformFees={calculatedPlatformFees} subTotalBeforeDiscount={subTotal} totalDiscount={discountAmount} totalTax={taxAmount} grandTotal={totalAmountDue} defaultTaxRatePercent={appConfig.visitingChargeTaxPercent || 0} />
                              <DialogClose asChild className="mt-2"><Button variant="outline" className="w-full">Close</Button></DialogClose>
                          </DialogContent>
                      </Dialog>
                 ...
                 {paymentMethod === 'Pay After Service' && payAfterServiceEnabled && (<p className="text-sm text-muted-foreground mt-1 text-right">You will be charged after service.</p>)}
              </div>
            </>
          )}
          {cartEntries.length === 0 && !isLoadingCartDetails && !isLoadingAppSettings && (<div className="text-center py-6"><p className="text-muted-foreground">Your cart is empty.</p><Link href="/cart" passHref className="mt-4 inline-block"><Button variant="outline">Return to Cart</Button></Link></div>)}
          {policyMessage && cartEntries.length > 0 && (<Alert variant="default" className="text-xs bg-primary/5 border-primary/20 mt-4"><Info className="h-4 w-4 text-primary" /><AlertDescription className="text-primary/90">{policyMessage}</AlertDescription></Alert>)}
        </CardContent>
        <CardFooter className="flex flex-col sm:flex-row justify-between gap-2 mt-4">
          <Link href="/checkout/address" passHref className="w-full sm:w-auto">
          ...
          <Button
            ...
            disabled={cartEntries.length === 0 || isLoadingCartDetails || isLoadingAppSettings || availablePaymentOptions.length === 0 || !paymentMethod || isProcessingPayment}
            className="w-full sm:w-auto"
          >
            {isProcessingPayment ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            {isProcessingPayment ? 'Processing...' : (paymentMethod === 'Pay After Service' ? 'Confirm Booking' : 'Confirm Booking & Pay')}
