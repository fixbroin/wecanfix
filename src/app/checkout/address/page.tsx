  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
        ...
        const loadProfileData = async (currentUser: any) => {
            ...
            try {
                ...
                if (docSnap.exists() && docSnap.data().mobileNumber) {
                    profileData.phone = docSnap.data().mobileNumber;
                }
            } catch (e) {
                console.error("Error fetching user's mobile from Firestore:", e);
            }
            return profileData;
        };
        ...
        const initializeForm = async () => {
                ...
                if (savedAddressRaw) {
                    try {
                        ...
                        if (savedAddress.email === user.email) {
                            initialFormData = { ...savedAddress }; // Prioritize saved checkout address for this user
                        } else {
                            localStorage.removeItem('fixbroCustomerAddress'); // Clear stale address
                        }
                    } catch (e) {
            ...
            // Ensure all fields have at least empty strings if not set
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <CardContent className="space-y-4 sm:space-y-6">
              ...
              {googleMapsApiKey && googleMapsApiKey.trim() !== "" ? (
                <Button type="button" variant="outline" className="w-full flex items-center" onClick={() => setIsMapModalOpen(true)}>
                  <SelectMapIcon className="mr-2 h-4 w-4 text-primary" /> Select Address via Map
                </Button>
              ) : (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="city" className="text-sm">City</FormLabel>
                        <FormControl>
                          <Input id="city" placeholder="e.g., Mumbai" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="state" className="text-sm">State</FormLabel>
                        <FormControl>
                          <Input id="state" placeholder="e.g., Maharashtra" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="pincode" className="text-sm">Pincode</FormLabel>
                      <FormControl>
                        <Input id="pincode" placeholder="e.g., 400001" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
