...
export async function generateMetadata(
  ...
  const areaData = cityData ? await getAreaData(cityData.id, params.area) : null;
  const categoryData = await getCategoryData(params.categorySlug);
  ...
  if (!cityData || !areaData || !categoryData) {
    ...
    if (!cityData) missingItems.push(`city "${params.city}"`);
    if (cityData && !areaData) missingItems.push(`area "${params.area}" (under city "${params.city}")`);
    else if (!cityData && !areaData) missingItems.push(`area "${params.area}" (city "${params.city}" also not found)`);
    if (!categoryData) missingItems.push(`category "${params.categorySlug}"`);
    
    // console.warn(`[AreaCategoryPage] generateMetadata: Content not found for ${missingItems.join(', ')}. Params: city=${params.city}, area=${params.area}, category=${params.categorySlug}`);
    return {
  ...
  const canonicalUrl = `${appBaseUrl}/${params.city}/${params.area}/${params.categorySlug}`;
...
export default async function AreaCategoryPage({ params }: AreaCategoryPageProps) {
  ...
  if (cityData) {
    breadcrumbItems.push({ label: cityData.name, href: `/${params.city}` });
    if (areaData) {
      breadcrumbItems.push({ label: areaData.name, href: `/${params.city}/${params.area}` });
    }
  }
  if (categoryData) {
  ...
  return <CategoryPageClient categorySlug={params.categorySlug} citySlug={params.city} areaSlug={params.area} breadcrumbItems={breadcrumbItems} />;
}
