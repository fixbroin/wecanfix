...
async function getAreaData(citySlug: string, areaSlug: string): Promise<(FirestoreArea & { parentCityData?: FirestoreCity }) | null> {
  try {
    ...
    if (citySnapshot.empty) {
      // console.warn(`City with slug "${citySlug}" not found or not active.`);
      return null;
    }
    ...
    if (areaSnapshot.empty) {
      // console.warn(`Area with slug "${areaSlug}" under city "${citySlug}" not found or not active.`);
      return null;
    }
...
export async function generateMetadata(
  ...
  const title = areaData.seo_title || replacePlaceholders(seoSettings.areaPageTitlePattern, placeholderData) || `${areaData.name}, ${areaData.parentCityData.name}${defaultSuffix}`;
  const description = areaData.seo_description || replacePlaceholders(seoSettings.areaPageDescriptionPattern, placeholderData) || `Find home services in ${areaData.name}, ${areaData.parentCityData.name}. ${seoSettings.defaultMetaDescription}`;
  const keywordsStr = areaData.seo_keywords || replacePlaceholders(seoSettings.areaPageKeywordsPattern, placeholderData) || seoSettings.defaultMetaKeywords;
...
export default async function AreaHomePage({ params }: AreaPageProps) {
  ...
  return (
    <>
      {/* The Breadcrumbs component is now only rendered by HomePageClient */}
