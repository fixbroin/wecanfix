...
export default function ChatWindow({ onClose }: ChatWindowProps) {
  ...
  console.log("[ChatWindow - Render Check] Current User:", currentUser, "Admin Profile:", adminProfile);
  ...
  useEffect(() => {
    const fetchAdminProfile = async () => {
        ...
        console.log("ChatWindow: Fetching admin profile...");
      if (globalSettings?.adminUserUidForChat) {
         console.log("ChatWindow: Using adminUserUidForChat from global settings:", globalSettings.adminUserUidForChat);
         ...
         if (adminDocSnap.exists()) {
            ...
            return;
             ...
             console.warn("ChatWindow: Admin UID from global settings not found. Falling back to ADMIN_EMAIL lookup.");
         }
      }
      ...
      if (!adminSnapshot.empty) {
        ...
        console.log("ChatWindow: Admin profile found by email:", { displayName: adminData.displayName, uid: adminUid });
        setAdminProfile({ displayName: adminData.displayName || null, photoURL: adminData.photoURL || null, uid: adminUid });
      } else {
        console.warn("ChatWindow: Admin profile with ADMIN_EMAIL not found. Using fallback admin_master_id (if configured) or hardcoded fallback.");
        setAdminProfile({ displayName: ADMIN_FALLBACK_NAME, photoURL: null, uid: 'admin_master_id' }); // Ensure this fallback UID is consistent or makes sense
      }
      setIsLoadingAdminProfile(false);
    };
  ...
  useEffect(() => {
    if (!currentUser || !chatSessionId || !adminProfile.uid || isLoadingAdminProfile) {
      console.log("ChatWindow: Conditions not met for message subscription. User:", !!currentUser, "SessionID:", chatSessionId, "AdminUID:", adminProfile.uid, "LoadingAdmin:", isLoadingAdminProfile);
      if(!isLoadingAdminProfile && !adminProfile.uid) setIsLoadingMessages(false);
      return;
    }
    console.log(`ChatWindow: Subscribing to messages for session ${chatSessionId}`);
    setIsLoadingMessages(true);
    const messagesRef = collection(db, 'chats', chatSessionId, 'messages');
    const q = query(messagesRef, orderBy('timestamp', 'asc'));

    const unsubscribe = onSnapshot(q, async (querySnapshot) => {
      console.log(`ChatWindow: Messages snapshot received for ${chatSessionId}, count: ${querySnapshot.docs.length}`);
      ...
      if (sessionSnap.exists()) {
      ...
      } else if (currentUser && adminProfile.uid) { // Ensure both UIDs are present before creating session
        console.log("ChatWindow: Creating new chat session document for", chatSessionId);
        await setDoc(sessionDocRef, {
            ...
            participants: [currentUser.uid, adminProfile.uid].filter(Boolean), // Ensure no null/undefined
        }, { merge: true });
      }

      const lastMessage = fetchedMessages[fetchedMessages.length -1];
      if (globalSettings?.chatNotificationSoundUrl && lastMessage && lastMessage.senderType === 'admin' && !lastMessage.isReadByUser && messages.length > 0) {
         // console.log("TODO: Play user notification sound for new admin message", globalSettings.chatNotificationSoundUrl);
      }
    ...
    return () => {
        console.log(`ChatWindow: Unsubscribing from messages for session ${chatSessionId}`);
        unsubscribe();
    };
  }, [currentUser, chatSessionId, adminProfile.uid, isLoadingAdminProfile, globalSettings?.chatNotificationSoundUrl, messages.length]);
  ...
  }, [messages, isLoadingMessages]); // Scroll on messages update and after loading

  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim() || !currentUser || !chatSessionId || !adminProfile.uid) {
         console.log("ChatWindow: Send message pre-condition failed. Message:", newMessage, "User:", !!currentUser, "SessionID:", chatSessionId, "AdminUID:", adminProfile.uid);
        return;
    }
    ...
    try {
      console.log(`ChatWindow: Attempting to send message for session ${chatSessionId}`);
      const messagesRef = collection(db, 'chats', chatSessionId, 'messages');
      ...
      console.log(`ChatWindow: Session doc updated for ${chatSessionId}`);
      ...
      if (adminProfile.uid && adminProfile.uid !== 'fallback_admin_uid' && adminProfile.uid !== 'admin_master_id') {
        ...
        console.log("ChatWindow: Admin notification created for", adminProfile.uid);
      }

      console.log("TODO: Send push notification for new user message to admin:", adminProfile.uid);
