...
import { MessageSquare, XIcon, Loader2 } from 'lucide-react'; // XIcon no longer needed here
import { useGlobalSettings } from '@/hooks/useGlobalSettings';
// ChatWindow import removed as it's no longer directly rendered here
...
export default function FrontendChatWidget() {
  ...
  const { user: currentUser, triggerAuthRedirect } = useAuth(); // Added triggerAuthRedirect
  const [userUnreadCount, setUserUnreadCount] = useState(0);
  const router = useRouter(); // Initialize useRouter
  const { showLoading } = useLoading(); // Initialize useLoading
  ...
  const handleChatButtonClick = () => {
    if (!currentUser) {
        triggerAuthRedirect('/chat'); // Redirect to login, then to chat page
        return;
    }
    if (!isLoadingAdminProfile) { // Only navigate if admin profile is loaded
        showLoading();
        router.push('/chat');
    }
  };
  ...
  return (
    <>
      <Button
        ...
        disabled={isLoadingAdminProfile && !currentUser} // Disable if loading admin and not logged in, or if user tries to open while admin details are loading
      >
      ...
      {/* ChatWindow is no longer rendered here */}
    </>
  );
}
