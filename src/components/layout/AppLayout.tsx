...
import CookieConsentBanner from '@/components/shared/CookieConsentBanner'; // Added CookieConsentBanner
...
const AppLayout: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    if (isClientMounted) { 
      ...
      const currentIsProviderRoute = pathname.startsWith('/provider'); // Added provider route check
      ...
      setShowHeader(!currentIsAdminRoute && !currentIsProviderRoute); // Hide for admin and provider
      
      setShowFooter(
        !currentIsAdminRoute &&
        !currentIsProviderRoute && // Hide for provider
        !currentIsAuthRoute &&
  ...
  useEffect(() => {
    if (isClientMounted && user && !authIsLoading) {
        ...
        const isProviderPage = pathname.startsWith('/provider/'); // Added provider page check
        if (!isAuthPage && !isAdminPage && !isProviderPage) { // Don't fetch reviews on provider pages either
            fetchPendingReview();
        }
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {isClientMounted && <CookieConsentBanner />} {/* Added CookieConsentBanner */}
    </div>
  );
};
