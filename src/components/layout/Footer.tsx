...
const FooterLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    showLoading(); // Show loader on click
    if (!user && href !== '/auth/login' && href !== '/auth/signup' && !href.startsWith('/admin')) {
      e.preventDefault();
      triggerAuthRedirect(href);
    } else {
      // Let Link component handle it by default, or router.push if more complex logic needed
      // If using Next's Link, router.push isn't strictly necessary here unless you have logic
      // that Link can't handle before navigation.
      // router.push(href); // This will also trigger pathname change and hide loader
    }
  };
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              {serviceLinks.length === 0 && !isLoadingCategories && ( 
                <>
                  <li><FooterLink href="#">Plumbing</FooterLink></li>
                  <li><FooterLink href="#">Electrical</FooterLink></li>
                  <li><FooterLink href="#">AC Services</FooterLink></li>
