...
import { Search, Bell, Menu, X, ShoppingCart, LogOut, UserCircle, Briefcase, Settings2, Moon, Sun, MessageSquare } from 'lucide-react';
...
const Header = () => {
  ...
  const handleProtectedNav = (e: React.MouseEvent<HTMLButtonElement | HTMLAnchorElement>, intendedHref: string) => {
    e.preventDefault();
    const currentClientPath = window.location.pathname + window.location.search + window.location.hash;

    if (intendedHref !== currentClientPath && !intendedHref.startsWith('#')) {
      showLoading();
    }

    if (!user && (intendedHref === '/chat' || intendedHref === '/profile' || intendedHref === '/my-bookings' || intendedHref === '/notifications')) {
  ...
  const authNavItemsGuest = [
      { href: '/auth/login', label: 'Login / Sign Up' }
  ];
      ...
      <header className="bg-background/80 backdrop-blur-md shadow-sm sticky top-0 z-50">
        <div data-hydration-state="hydrated" className="container mx-auto px-4 h-16 grid grid-cols-header-layout items-center">
          <div className="flex items-center justify-start">
                  ...
                  <ScrollArea className="flex-grow">
                    <nav className="flex flex-col gap-1 p-4">
                      {baseNavItems.map((item) => (
                        <NavLink key={item.href} href={item.href} onClick={(e) => handleProtectedNav(e, item.href)}>
                          {item.label}
                        </NavLink>
                      ))}
                      {user && (
                        <NavLink href="/my-bookings" onClick={(e) => handleProtectedNav(e, "/my-bookings")}>
                          <Briefcase className="mr-2 inline-block h-4 w-4" /> My Bookings
                        </NavLink>
                      )}
                      {chatIsEnabled && (
                        <NavLink href="/chat" onClick={(e) => handleProtectedNav(e, "/chat")}>
                          <MessageSquare className="mr-2 inline-block h-4 w-4" /> Chat with Support
                        </NavLink>
                      )}
                      {user ? authNavItemsUser.map((item) => (
                        <NavLink key={item.href} href={item.href} onClick={(e) => handleProtectedNav(e, item.href)}>
                          <item.icon className="mr-2 inline-block h-4 w-4" /> {item.label}
                        </NavLink>
                      )) : authNavItemsGuest.map((item) => (
                        <NavLink key={item.href} href={item.href} onClick={(e) => handleProtectedNav(e, item.href)}>
                          {item.label}
                        </NavLink>
                      ))}
                      {user && user.email === ADMIN_EMAIL && (
                          <NavLink href="/admin" onClick={(e) => handleProtectedNav(e, "/admin")}>
            ...
            <nav className="hidden md:flex items-center gap-x-3 lg:gap-x-5">
              {baseNavItems.map((item) => (
               ...
               {user && (
                <button
                  onClick={(e) => handleProtectedNav(e, '/my-bookings')}
                  className="text-sm font-medium text-foreground/80 hover:text-primary transition-colors"
                >
                  My Bookings
                </button>
          ...
          <div className="flex items-center justify-end gap-1 md:gap-2">
            ...
            <Button variant="ghost" size="icon" aria-label="Notifications" onClick={(e) => handleProtectedNav(e, '/notifications')} className="relative">
            ...
            <Button variant="ghost" size="icon" aria-label="Cart" className="relative" onClick={(e) => handleProtectedNav(e, '/cart')}>
            ...
            <div className="hidden md:block">
               {!user && !authIsLoading && (
                 <Button variant="outline" size="sm" onClick={(e) => handleProtectedNav(e, '/auth/login')}>Login / Sign Up</Button>
               )}
               {user && (
                 <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-56" align="end" forceMount>
                      ...
                      <DropdownMenuSeparator />
                      {authNavItemsUser.map(item => (
                        <DropdownMenuItem key={item.href} asChild>
                          <Link href={item.href} onClick={(e) => handleProtectedNav(e, item.href)}>
                      ...
                      {user.email === ADMIN_EMAIL && (
                        <DropdownMenuItem asChild>
                          <Link href="/admin" onClick={(e) => handleProtectedNav(e, "/admin")}>
