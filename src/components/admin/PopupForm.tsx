...
import { storage } from '@/lib/firebase'; // For deleting old images if needed
...
export default function PopupForm({ onSubmit: onSubmitProp, initialData, onCancel, isSubmitting: isParentSubmitting = false }: PopupFormProps) {
    ...
    defaultValues: initialData ? 
      {
        ...
        showEmailInput: false, promoCode: "", targetUrl: "", displayRuleType: "on_page_load",
  ...
  useEffect(() => {
    // This effect is now primarily for image preview and file input reset when initialData changes.
    // The form's defaultValues are handled by the key on the <Form> component.
    if (initialData) {
      setCurrentImagePreview(initialData.imageUrl || null);
      setOriginalImageUrlFromInitialData(initialData.imageUrl || null);
      // Reset specific fields not directly part of initialData's structure if needed,
      // but useForm's defaultValues (keyed by initialData.id) should handle most cases.
      // For select dropdowns, if initialData.popupType is valid, it's used by useForm's defaultValues.
      form.reset( // Explicitly reset if initialData changes after initial mount (e.g. editing different items)
        {
      ...
      form.reset( // Reset to blank form defaults
        {
            name: "", popupType: "newsletter_signup", title: "", displayText: "", imageUrl: "", imageHint: "", videoUrl: "",
            showEmailInput: false, promoCode: "", targetUrl: "", displayRuleType: "on_page_load",
            displayRuleValue: null, displayFrequency: "once_per_session", showCloseButton: true, isActive: true,
    ...
    try {
      if (selectedFile) {
        ...
        if (originalImageUrlFromInitialData && isFirebaseStorageUrl(originalImageUrlFromInitialData)) {
          ...
          catch (error) { console.warn("Error deleting old Firebase image: ", error); }
        }
        const fileUploadData = new FormData();
        fileUploadData.append('file', selectedFile);
        fileUploadData.append('uploadPath', 'popups');

        const response = await fetch('/api/upload', { method: 'POST', body: fileUploadData });
        const result = await response.json();
        if (!response.ok || !result.success) throw new Error(result.error || result.message || 'Image upload via API failed.');
        finalImageUrl = result.url;
        setUploadProgress(100); setStatusMessage("Image uploaded. Saving popup...");
        ...
        try { await deleteObject(storageRef(storage, originalImageUrlFromInitialData)); }
        catch (error) { console.warn("Error deleting old Firebase image on clear: ", error); }
        finalImageUrl = ""; setStatusMessage("Image reference removed. Saving popup...");
      } else {
        setStatusMessage(initialData ? "Saving changes..." : "Creating popup...");
      }

      const payload = { ...formData, imageUrl: finalImageUrl, id: initialData?.id };
  ...
  return (
    <Form {...form} key={initialData ? `popup-form-${initialData.id}` : 'new-popup-form'}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="flex-grow space-y-6 p-6 overflow-y-auto">
        ...
        {(watchedPopupType === 'newsletter_signup' || watchedPopupType === 'lead_capture' || watchedPopupType === 'subscribe') && (
            <FormField control={form.control} name="showEmailInput" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm"><div className="space-y-0.5"><FormLabel>Show Email Input Field</FormLabel><FormDescription>Include an email capture form in the popup.</FormDescription></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} disabled={effectiveIsSubmitting} /></FormControl></FormItem>)} />
        )}
