...
interface UserDetailsModalProps {
  user: FirestoreUser;
  onClose: () => void;
  onUpdateUser: (updatedData: Partial<FirestoreUser>) => Promise<boolean>; // Returns true on success
}
...
const userEditSchema = z.object({
  ...
  mobileNumber: z.string()
    ...
    .optional().or(z.literal('')), // Optional, allow empty string
});
...
export default function UserDetailsModal({ user, onClose, onUpdateUser }: UserDetailsModalProps) {
  ...
  const onSubmit = async (data: UserEditFormData) => {
    ...
    const success = await onUpdateUser({
      ...
      email: data.email,
      mobileNumber: data.mobileNumber || null, // Send null if empty string for Firestore
    });
  ...
  const formatTimestamp = (timestamp?: any): string => {
    ...
    if (timestamp.toDate && typeof timestamp.toDate === 'function') {
      return timestamp.toDate().toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' });
    }
    try {
            ...
            {isEditing ? (
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  ...
                  <div><strong>Created At:</strong> {formatTimestamp(user.createdAt)}</div>
                  <div><strong>Last Login:</strong> {formatTimestamp(user.lastLoginAt)}</div>
                   <div>
