...
export default function AreaCategorySeoForm({ onSubmit: onSubmitProp, initialData, cities, areas, categories, onCancel, isSubmitting = false }: AreaCategorySeoFormProps) {
  ...
  useEffect(() => {
    if (watchedCityId) {
      ...
      // Only reset areaId if the city has actually changed from the initial data's city,
      // or if it's a new form.
      ...
      setFilteredAreas([]);
      if (!initialData) { // Only reset if it's a new form and cityId is cleared
          form.setValue('areaId', undefined, { shouldValidate: true });
      }
    }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <FormField control={form.control} name="areaId" render={({ field }) => (
          <FormItem><FormLabel>Area</FormLabel>
            <Select
              ...
              onValueChange={field.onChange}
              value={field.value || undefined}
              disabled={isSubmitting || isEditing || !watchedCityId || filteredAreas.length === 0}
            >
        ...
        <FormField control={form.control} name="slug" render={({ field }) => (
          <FormItem><FormLabel>Slug Segment {isEditing ? "(Non-editable)" : "(Auto-generated or custom)"}</FormLabel><FormControl><Input placeholder="e.g., bangalore/whitefield/plumbing" {...field} value={field.value || ""} onChange={(e) => field.onChange(generateSeoSlug(e.target.value.split('/')))} disabled={isSubmitting || isEditing} /></FormControl><FormDescription>Final URL uses original city/area/category slugs. This is for internal reference.</FormDescription><FormMessage /></FormItem>
        )}/>
        <FormField control={form.control} name="h1_title" render={({ field }) => (<FormItem><FormLabel>H1 Title</FormLabel><FormControl><Input placeholder="e.g., Plumbing in Whitefield, Bangalore" {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>)}/>
        <FormField control={form.control} name="meta_title" render={({ field }) => (<FormItem><FormLabel>Meta Title</FormLabel><FormControl><Input placeholder="e.g., Plumbers Whitefield, Bangalore | FixBro" {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>)}/>
        <FormField control={form.control} name="meta_description" render={({ field }) => (<FormItem><FormLabel>Meta Description</FormLabel><FormControl><Textarea placeholder="Find expert plumbers in Whitefield, Bangalore..." {...field} value={field.value || ""} rows={3} /></FormControl><FormMessage /></FormItem>)}/>
        <FormField control={form.control} name="meta_keywords" render={({ field }) => (<FormItem><FormLabel>Meta Keywords (comma-separated)</FormLabel><FormControl><Input placeholder="e.g., plumbers whitefield, whitefield plumbing" {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>)}/>
        <FormField control={form.control} name="imageHint" render={({ field }) => (<FormItem><FormLabel>Image Hint (Optional)</FormLabel><FormControl><Input placeholder="e.g., plumber working area" {...field} value={field.value || ""} /></FormControl><FormDescription>Keywords for OG image if specific image isn't set.</FormDescription><FormMessage /></FormItem>)}/>
