...
const slideFormSchema = z.object({
  ...
  imageUrl: z.string().optional().or(z.literal('')), // Will be validated in handleSubmit
  ...
  buttonLinkType: z.enum(['category', 'subcategory', 'service', 'url']).optional(),
  buttonLinkValue: z.string().max(255, "Link value is too long.").optional().or(z.literal('')),
  isActive: z.boolean().default(true),
}).refine(data => {
  // If buttonText is provided, then buttonLinkType and buttonLinkValue are required.
  if (data.buttonText && data.buttonText.trim() !== "") {
    return !!data.buttonLinkType && !!(data.buttonLinkValue && data.buttonLinkValue.trim() !== "");
  }
  // If buttonText is not provided, image can still be a link, so buttonLinkType/Value can be set.
  // If buttonLinkType OR buttonLinkValue is set (but not both), it's an issue ONLY if buttonText is also set.
  // This specific scenario is covered by the first part of the condition.
  // If no buttonText, then buttonLinkType and buttonLinkValue being partially set is fine as it might be WIP.
  // The main requirement is if buttonText is there, the others must be too.
  return true;
}, {
  message: "If Button Text is present, Link Type and Link Value are required.",
  path: ["buttonLinkValue"], // Or a more general path if preferred
});
...
export default function SlideshowForm({
  ...
  useEffect(() => {
    if (initialData) {
      form.reset({
        ...
        buttonLinkType: initialData.buttonLinkType,
  ...
  const handleSubmit = async (formData: SlideFormData) => {
    ...
    try {
      ...
      const payload: Omit<FirestoreSlide, 'id' | 'createdAt' | 'updatedAt'> = {
        ...
        buttonLinkType: (formData.buttonText && formData.buttonText.trim() !== "") || (formData.buttonLinkType && formData.buttonLinkValue?.trim() !== "") ? formData.buttonLinkType : undefined,
        buttonLinkValue: (formData.buttonText && formData.buttonText.trim() !== "") || (formData.buttonLinkType && formData.buttonLinkValue?.trim() !== "") ? formData.buttonLinkValue : undefined,
        isActive: formData.isActive === undefined ? true : formData.isActive,
      };
      
      // Ensure buttonLinkType and buttonLinkValue are cleared if buttonText is empty AND they are not meant for image link
      if (!payload.buttonText && !(payload.buttonLinkType && payload.buttonLinkValue)) {
          payload.buttonLinkType = undefined;
          payload.buttonLinkValue = undefined;
      }
  ...
  // Show asterisk for link type/value if button text is present OR if a link type is already selected (for image link)
            ...
            <FormItem>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <FormControl className="flex-grow">
                  <Textarea
                    placeholder="https://example.com/image.png. Clear to remove image."
                    {...field}
        ...
        <FormField
            ...
            render={({ field }) => (
                <FormItem>
                    ...
                    <Select onValueChange={(value) => { field.onChange(value as SlideButtonLinkType); form.setValue('buttonLinkValue', ''); }} value={field.value} disabled={effectiveIsSubmitting}>
        ...
        <FormField
            ...
            render={({ field }) => (
                <FormItem>
                    ...
                    {watchedLinkType === 'url' && <FormControl><Input placeholder="https://example.com/offer" {...field} disabled={effectiveIsSubmitting || !watchedLinkType} /></FormControl>}
                    {watchedLinkType === 'category' && (
                        <Select onValueChange={field.onChange} value={field.value} disabled={effectiveIsSubmitting || categories.length === 0 || !watchedLinkType}>
                    ...
                    {watchedLinkType === 'subcategory' && (
                         <Select onValueChange={field.onChange} value={field.value} disabled={effectiveIsSubmitting || subCategories.length === 0 || !watchedLinkType}>
                    ...
                    {watchedLinkType === 'service' && (
                        <Select onValueChange={field.onChange} value={field.value} disabled={effectiveIsSubmitting || services.length === 0 || !watchedLinkType}>
