  ...
  useEffect(() => {
    if (initialData) {
      ...
      form.reset({
        ...
        isTaxInclusive: initialData.isTaxInclusive === true ? "true" : "false", // Corrected initialization
  ...
  useEffect(() => {
    ...
    if (currentParentId) {
      ...
      if (currentSubCatId && !newFiltered.find(sc => sc.id === currentSubCatId)) {
        // Only reset if the current subCategory is no longer valid for the NEW parent
        // This check might need to be more sophisticated if initialData.subCategoryId should be preserved
        // across parent changes during editing (which is unusual UI).
        // For "Add New", this is fine. For "Edit", if parent changes, subCat MUST change.
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select
                key={`parent-cat-select-${initialData?.id || 'new-service'}-${parentCategories.length}-${field.value}`}
                onValueChange={(value) => {
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder={parentCategories.length > 0 ? "Select a parent category" : "No parent categories"} />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select
                key={`subcat-select-${initialData?.id || 'new-service'}-${watchedParentCategoryId}-${filteredSubCategories.length}-${field.value}`}
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder={!watchedParentCategoryId ? "Select parent category first" : (filteredSubCategories.length > 0 ? "Select a sub-category" : "No sub-categories for selected parent")} />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
            ...
            render={({ field }) => {
              return (
                <FormItem>
                  ...
                  <Select
                    key={`is-tax-inclusive-select-${initialData?.id || 'new-service'}-${taxes.length}-${String(field.value)}`} 
                    onValueChange={field.onChange} 
                    value={field.value} // Value for Select should be a string
