...
import { Loader2, Image as ImageIcon, Trash2, PlusCircle, Percent, Clock, HelpCircle } from "lucide-react";
...
export default function ServiceForm({ onSubmit: onSubmitProp, initialData, onCancel, parentCategories, subCategories, taxes, isSubmitting: isParentSubmitting = false }: ServiceFormProps) {
  ...
  const { fields: highlightFields, append: appendHighlight, remove: removeHighlight } = useFieldArray({ control: form.control, name: "serviceHighlights" });
  const { fields: includedFields, append: appendIncluded, remove: removeIncluded } = useFieldArray({ control: form.control, name: "includedItems" });
  const { fields: excludedFields, append: appendExcluded, remove: removeExcluded } = useFieldArray({ control: form.control, name: "excludedItems" });
  const { fields: faqFields, append: appendFaq, remove: removeFaq } = useFieldArray({ control: form.control, name: "serviceFaqs" });
    ...
    <Form {...form} key={initialData ? `service-form-${initialData.id}` : 'new-service-form'}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="flex-grow space-y-6 p-6 overflow-y-auto">
        ...
        <div>
            ...
            <FormField control={form.control} name="fullDescription" render={({ field }) => (<FormItem className="mt-2"><FormLabel>Please Note (Optional)</FormLabel><FormControl><Textarea placeholder="Detailed info, what's included, etc. (Old 'What's Included' field)" {...field} value={field.value ?? ""} rows={5} disabled={effectiveIsSubmitting}/></FormControl><FormMessage /></FormItem>)}/>
        </div>
        {/* Included Items */}
        <Separator />
        <div>
