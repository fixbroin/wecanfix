  ...
  useEffect(() => {
    ...
    const fetchFeaturesConfigAndData = async () => {
        ...
        if (currentFeaturesConfig.showRecentlyAddedServices) {
            ...
            try {
                const servicesQuery = query(collection(db, "adminServices"), where("isActive", "==", true), orderBy("createdAt", "desc"), limit(10));
        ...
        if (currentFeaturesConfig.showMostPopularServices) {
            ...
            try {
                ...
                const servicesQuery = query(collection(db, "adminServices"), where("isActive", "==", true), orderBy("rating", "desc"), orderBy("reviewCount", "desc"), limit(10));
                const snapshot = await getDocs(servicesQuery);
                ...
                if (enabledCategoryIds.length > 0) {
                    ...
                    const categoryServicesPromises = enabledCategories.map(async (cat) => {
                        ...
                        if (subCategoryIds.length > 0) {
                             const servicesQuery = query(collection(db, "adminServices"), where("isActive", "==", true), where("subCategoryId", "in", subCategoryIds), orderBy("name", "asc"), limit(5));
