...
import type { FirestoreService, FirestoreReview, ClientServiceData, FirestoreCategory, FirestoreSubCategory } from '@/types/firestore';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Star, ShoppingCart, ArrowLeft, Home as HomeIcon, CheckCircle, ShieldCheck, Clock, Loader2, MessageSquare } from 'lucide-react';
        ...
        <div className="grid md:grid-cols-2 gap-0">
          <div className="relative w-full h-[180px] sm:h-[250px] md:h-[300px] lg:h-[400px] xl:h-[450px] overflow-hidden md:rounded-l-lg">
            <Image
              ...
              priority // Prioritize loading of main service image
              onError={(e) => {
          ...
          <div className="flex flex-col">
            <CardHeader className="p-4 sm:p-6">
              <div className="flex items-start justify-between">
                <CardTitle className="text-2xl sm:text-3xl font-headline text-foreground mb-1">{h1Title}</CardTitle>
              ...
              {service.rating > 0 && (
                <div className="flex items-center text-xs sm:text-sm">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 sm:h-5 sm:w-5 ${i < Math.floor(service.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/30'}`}
                    />
                  ))}
                  <span className="ml-1 sm:ml-2 text-muted-foreground">({service.rating.toFixed(1)} rating)</span>
                  {service.reviewCount !== undefined && service.reviewCount > 0 && (
                     <span className="ml-1 sm:ml-2 text-muted-foreground">· {service.reviewCount} reviews</span>
                  )}
                </div>
              )}
            </CardHeader>
            ...
            <CardContent className="p-4 sm:p-6 pt-0 flex-grow space-y-3 sm:space-y-4">
              ...
              {service.shortDescription && (
                 <p className="text-xs sm:text-sm text-muted-foreground">{service.shortDescription}</p>
              )}
                  ...
                  <ul className="list-none space-y-0.5 sm:space-y-1 text-xs sm:text-sm text-muted-foreground">
                    {service.serviceHighlights.map((highlight, index) => (
                      <li key={index} className="flex items-center">
                        <CheckCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-2 text-accent flex-shrink-0"/> {highlight}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {service.fullDescription && (
                <>
                  <Separator/>
                  <div>
                    <h4 className="text-sm sm:text-md font-semibold text-foreground mb-1">More Details</h4>
                    <p className="text-xs sm:text-sm text-muted-foreground whitespace-pre-wrap">{service.fullDescription}</p>
                  </div>
                </>
              )}
              <Separator />
              <div>
                {service.discountedPrice && service.discountedPrice < service.price ? (
                  <div className="flex items-baseline gap-1 sm:gap-2">
                    ...
                    {priceSaved > 0 &&
                      <Badge variant="destructive" className="text-xs">SAVE ₹{priceSaved.toLocaleString()} ({Math.round((priceSaved / service.price) * 100)}%)</Badge>
                    }
                  </div>
                ) : (
      ...
      {(serviceReviews.length > 0 || isLoadingReviews) && (
        <Card className="shadow-lg mt-6 sm:mt-8">
            ...
            {isLoadingReviews ? (
                ...
                {[...Array(2)].map((_, i) => (
                    <div key={i} className="p-3 sm:p-4 border rounded-md">
                    <div className="flex items-center justify-between mb-1 sm:mb-2">
                        <Skeleton className="h-4 sm:h-5 w-1/3" />
                        <Skeleton className="h-3 sm:h-4 w-12" />
                    </div>
                    <Skeleton className="h-3 sm:h-4 w-1/4 mb-1 sm:mb-2" />
                ...
                {serviceReviews.map(review => (
                    ...
                    <div className="flex items-center justify-between mb-1 sm:mb-1.5">
                        ...
                        {[...Array(5)].map((_, i) => (
                            <Star
                            key={i}
                            className={`h-4 w-4 sm:h-5 sm:w-5 ${i < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/30'}`}
                            />
                        ))}
                        </div>
                    </div>
